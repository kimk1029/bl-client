...
export default function PinnedNotice({ post }: { post: Post | null }) {
  if (!post) {
    ...
    return (
      <div className="blessing-pinned" aria-label={`샘플 공지: ${sample.title}`}>
        <PinIcon />
        <strong>공지</strong>
        <span className="blessing-pinned-dot">·</span>
        <span className="blessing-pinned-title">{sample.title}</span>
        <span className="blessing-pinned-time">{sample.timeAgo}</span>
      </div>
    );
  }
  return (
