...
export default function NoticeSection({ posts, excludeIds }: Props) {
  ...
  const showingSamples = items.length === 0;
      ...
      <div className="blessing-notice-list">
        {showingSamples
          ? SAMPLE_NOTICES.map((n) => (
              <div
                key={n.id}
                className="blessing-notice-item blessing-notice-sample"
                aria-label="샘플 공지"
              >
                <span
                  className={`blessing-notice-badge${n.pinned ? " blessing-notice-badge-pin" : ""}`}
                >
                  {n.pinned ? "공지" : "샘플"}
                </span>
                <span className="blessing-notice-title">{n.title}</span>
                <span className="blessing-notice-time">{n.timeAgo}</span>
              </div>
            ))
          : items.map((p) => {
